...
in_pre = False
with open(output_file, 'w', encoding="utf-8") as _out:
    with open(input_file, encoding="utf-8") as _in:
        for line in _in:
            # round-robin image servers
            #if "<img src=i/" in line:
            #    line = line.replace("<img src=i/", "<img src=http://" + next(available_server) + "/dip3-it/")
            #if "url(i/" in line:
            #    line = line.replace("url(i/", "url(http://" + next(available_server) + "/dip3-it/")
